-- Give Python Development Pro Bootcamp the same 30-row syllabus as
-- Advanced Python Development. Repeat-safe: existing days are updated and
-- missing days are inserted.
...
with source_course as (
  select id
  from public.courses
  where lower(btrim(title)) = lower('Advanced Python Development')
  order by created_at asc
  limit 1
), target_course as (
  select id
  from public.courses
  where lower(btrim(title)) = lower('Python Development Pro Bootcamp')
  order by created_at asc
  limit 1
)

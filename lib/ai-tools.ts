...
export type AITool = {
  id: string; name: string; url: string; benefits: string; image_url: string;
...
export function validateAITool(input: { name: string; url: string; benefits: string; image_url: string }) {
  const data = { name: input.name.trim(), url: input.url.trim(), benefits: input.benefits.trim(), image_url: input.image_url.trim() };

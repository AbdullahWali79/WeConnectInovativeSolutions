...
export type AITool = {
  id: string; name: string; url: string; category: string; benefits: string; image_url: string; youtube_url: string | null;
...
export function validateAITool(input: { name: string; url: string; category: string; benefits: string; image_url: string; youtube_url?: string }) {
  const data = { name: input.name.trim(), url: input.url.trim(), category: input.category.trim(), benefits: input.benefits.trim(), image_url: input.image_url.trim(), youtube_url: input.youtube_url?.trim() || null };
  if (data.name.length < 2 || data.name.length > 120) return { ok: false as const, error: "Tool name must be 2 to 120 characters." };
  if (data.category.length < 2 || data.category.length > 100) return { ok: false as const, error: "Category must be 2 to 100 characters." };

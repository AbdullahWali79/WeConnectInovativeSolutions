...
export async function GET() {
  try {
    const [settings, deepSeek] = await Promise.all([getAiSettings(), getDeepSeekSettings().catch(() => null)]);
    const enabled = (settings.enabled && Boolean(settings.api_key)) || Boolean(deepSeek?.enabled && deepSeek.api_key);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!geminiAvailable && !deepSeekAvailable) return NextResponse.json({ error: "The AI assistant is currently offline. Please use the Contact page." }, { status: 503 });

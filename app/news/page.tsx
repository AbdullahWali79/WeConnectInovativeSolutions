...
export default async function NewsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#030B1C] text-white">
        ...
        <div className="relative z-10 mx-auto max-w-container-max px-5 md:px-margin-page text-center md:text-left">
          <FadeIn>
            <div className="max-w-3xl">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-6xl font-black leading-tight text-white mb-6">Latest in AI & Tech</h1>
              <p className="text-lg leading-relaxed text-[#91A3C7]">
      ...
      {/* News Section */}
      <section className="relative bg-[#030B1C] py-16 md:py-24">
          ...
          {news.length > 0 ? (
            <StaggerContainer className="grid gap-6 md:grid-cols-2 lg:grid-cols-3" staggerDelay={0.1}>
              {news.map((item) => (
                <StaggerItem key={item.id}>
                  <div className="group flex flex-col h-full overflow-hidden rounded-3xl border border-white/10 bg-[#061A3D]/60 backdrop-blur-xl p-6 shadow-[0_0_50px_rgba(0,0,0,0.3)] transition-all duration-500 hover:-translate-y-2 hover:border-[#FFD24A]/30 hover:bg-[#061A3D]/80 hover:shadow-[0_0_30px_rgba(255,210,74,0.15)]">
                    <div className="mb-6 flex items-center justify-between">
                      <span className="rounded-full bg-[#FFD24A]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#FFD24A]">
                        {item.source}
                      </span>
                      <Icon
                        name="newspaper"
                        className="text-[#91A3C7] opacity-50 transition-opacity duration-300 group-hover:text-[#FFD24A] group-hover:opacity-100"
                      />
                    </div>
                    <h3 className="mb-4 text-xl font-black leading-tight text-white line-clamp-2 transition-colors group-hover:text-[#FFD24A]">
                      {item.title}
                    </h3>
                    <p className="mb-6 flex-1 text-sm leading-relaxed text-[#91A3C7] line-clamp-3">{item.summary}</p>
                    <div className="mt-auto border-t border-white/10 pt-5">
                      <a
                        href={item.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 text-sm font-bold text-white transition-all group-hover:text-[#FFD24A]"
                      >
            ...
            <FadeIn>
              <div className="rounded-3xl border border-dashed border-white/20 bg-white/5 p-12 text-center backdrop-blur-md">
                <Icon name="newspaper" className="mx-auto text-5xl text-[#91A3C7]" />
                <h3 className="mt-5 text-2xl font-black text-white">News is being fetched</h3>
                <p className="mt-2 text-[#91A3C7]">Refresh the page in a moment to see the latest AI and tech news.</p>
              </div>
            </FadeIn>
          )}
        </div>
      ...
      {/* CTA Section */}
      <section className="relative overflow-hidden bg-[#061A3D]/40 py-20 md:py-24 text-center border-t border-white/5">
        ...
        <div className="relative z-10 mx-auto max-w-container-max px-5 md:px-margin-page">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-6">Stay Updated With Our News</h2>
          <p className="text-lg leading-relaxed text-[#91A3C7] max-w-2xl mx-auto mb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            ...
            <Link href="/" className="w-full sm:w-auto inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/5 px-10 py-4 text-sm font-bold text-white transition-colors hover:bg-white/10 hover:border-[#FFD24A]/30">
              BACK TO HOME
            </Link>
          </div>
        </div>

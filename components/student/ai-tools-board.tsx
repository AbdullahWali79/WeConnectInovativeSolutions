...
export function AIToolsBoard() {
  ...
  const STANDARD_CATEGORIES = ["Video Editing", "Image Generation", "Text & Writing", "Coding", "Productivity", "Audio & Music", "Design & UI", "Other"];
  ...
  return <div className="space-y-6"><Toast toast={toast} onClear={() => setToast(null)} /><PageHeader eyebrow="Community research" title="AI Tools" description="Share a useful AI tool. It will be published after admin verification." />
    <form onSubmit={submit} className="wc-card grid gap-4 p-5 md:grid-cols-2">
      <label className="space-y-1"><span className="wc-label">Tool name *</span><input required maxLength={120} className="wc-input" value={form.name} onChange={e => setForm({...form,name:e.target.value})} placeholder="e.g. Perplexity AI" /></label>
      <label className="space-y-1"><span className="wc-label">Category *</span>

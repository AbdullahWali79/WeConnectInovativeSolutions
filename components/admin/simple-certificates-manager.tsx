...
const blank = (): SimpleCertificateInput => ({ roll_number: "", student_name: "", course_name: "", duration_weeks: 8, start_date: today(), end_date: plusWeeks(today(), 8), software_house_id: null, software_house_name: "", logo_url: null, signatory_name: "", signatory_title: "", primary_color: "#1455d9", secondary_color: "#338ed8", text_color: "#244b5e" });
...
export function SimpleCertificatesManager() {
  ...
  function edit(row: SimpleCertificate) { setEditing(row.id); setForm({ roll_number: row.roll_number, student_name: row.student_name, course_name: row.course_name, duration_weeks: row.duration_weeks, start_date: row.start_date, end_date: row.end_date, software_house_id: row.software_house_id, software_house_name: row.software_house_name, logo_url: row.logo_url, signatory_name: row.signatory_name, signatory_title: row.signatory_title, primary_color: row.primary_color || "#1455d9", secondary_color: row.secondary_color || "#338ed8", text_color: row.text_color || "#244b5e" }); window.scrollTo({ top: 0, behavior: "smooth" }); }
...
const CertificatePreview = ({ data }: { data: SimpleCertificateInput }) => <div className="certificate-print relative mx-auto aspect-[1.414/1] w-full max-w-[1100px] overflow-hidden bg-white p-[6%] shadow-xl print:max-w-none print:shadow-none" style={{ color: data.text_color }}>
  <div className="absolute left-0 top-0 h-24 w-24" style={{ backgroundColor: data.primary_color }} /><div className="absolute left-12 top-0 h-16 w-16 opacity-90" style={{ backgroundColor: data.secondary_color }} /><div className="absolute bottom-0 right-0 h-20 w-20" style={{ backgroundColor: data.primary_color }} /><div className="absolute bottom-10 right-0 h-16 w-16 opacity-90" style={{ backgroundColor: data.secondary_color }} />
  <div className="absolute inset-[7%] border-2" style={{ borderColor: data.primary_color }} />
  <div className="relative z-10 flex h-full flex-col items-center justify-center text-center">
    ...
    <h2 className="my-4 text-3xl font-black sm:text-6xl" style={{ color: data.text_color }}>{data.student_name || "Student Name"}</h2>
    ...
    <div className="mt-10 w-[42%] min-w-56 self-end text-center"><div className="border-b-2 pb-2 font-bold" style={{ borderColor: data.primary_color }}>{data.signatory_name || "Signature"}</div><p className="mt-2 text-xs font-semibold">Head of {data.software_house_name || "Software House"}</p></div>

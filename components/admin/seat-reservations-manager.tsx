...
export function SeatReservationsManager() {
  ...
  return <>
    ...
    <PageHeader eyebrow="Seat management" title="Seat reservations" description="Create time slots, monitor the 20-seat capacity, check students in, and manage no-show fines." action={<div className="flex flex-wrap gap-2"><button type="button" onClick={() => setOpenModal("holiday")} className="wc-secondary-btn"><Icon name="event_busy" /> Announce Holiday</button><button type="button" onClick={() => setOpenModal("settings")} className="wc-secondary-btn"><Icon name="settings" /> Reservation Settings</button><button type="button" onClick={() => setOpenModal("slot")} className="wc-primary-btn"><Icon name="add" /> Create Time Slot</button></div>} />

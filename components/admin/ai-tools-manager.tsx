...
export function AIToolsManager() {
  ...
  return <div className="space-y-8">
    ...
    <CollapsibleToolSection title="Published Tools" subtitle="Tools that are currently live and visible to students." rows={publishedTools} defaultOpen renderActions={(row, close) => <><button onClick={() => { close(); startEdit(row); }} className="wc-secondary-btn text-sm"><Icon name="edit" />Edit</button><button onClick={() => { close(); void editVideo(row); }} className="wc-secondary-btn text-sm"><Icon name="play_circle" />{row.youtube_url ? "Video" : "Video"}</button><button onClick={() => { close(); void remove(row.id); }} className="wc-secondary-btn text-sm text-error"><Icon name="delete" />Delete</button></>} />
    ...
    <section>
      ...
      <CollapsibleToolSection title={`${studentFilter === "all" ? "All" : studentFilter[0].toUpperCase() + studentFilter.slice(1)} Unpublished Tools`} subtitle="Click a card to view complete details and review it." rows={shownPending} defaultOpen renderActions={(row, close) => <><button onClick={() => { close(); startEdit(row); }} className="wc-secondary-btn text-sm"><Icon name="edit" />Edit</button>{row.status !== "approved" && <button onClick={() => { close(); void review(row.id, "approved"); }} className="wc-primary-btn text-sm"><Icon name="check" />Approve & Publish</button>}{row.status !== "rejected" && <button onClick={() => { close(); void review(row.id, "rejected"); }} className="wc-secondary-btn text-sm"><Icon name="close" />Reject</button>}<button onClick={() => { close(); void remove(row.id); }} className="wc-secondary-btn text-sm text-error"><Icon name="delete" />Delete</button></>} />
    </section>
  </div>;
}

function CollapsibleToolSection({ title, subtitle, rows, defaultOpen = false, renderActions }: { title: string; subtitle: string; rows: AITool[]; defaultOpen?: boolean; renderActions: (row: AITool, close: () => void) => React.ReactNode }) {

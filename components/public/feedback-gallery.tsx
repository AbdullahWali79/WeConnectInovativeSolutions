...
function StarRow({ rating }: { rating: number }) {
  return (
    <div className="flex gap-1 text-[#FFD24A]">
  ...
  return (
    <article
      className={`group relative h-full overflow-hidden rounded-[30px] border transition-all duration-700 ${
        active
          ? "border-[#FFD24A]/45 bg-[linear-gradient(180deg,#0b1c3d_0%,#08122a_100%)] shadow-[0_24px_80px_rgba(2,7,27,0.45)]"
          : "border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.14)_0%,rgba(255,255,255,0.08)_100%)] shadow-[0_18px_54px_rgba(2,7,27,0.22)]"
      }`}
    >
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute -right-20 top-10 h-40 w-40 rounded-full bg-[#FFD24A]/10 blur-3xl" />
        <div className="absolute -left-16 bottom-0 h-40 w-40 rounded-full bg-[#062b7f]/18 blur-3xl" />
      </div>
      ...
      <div className="relative flex h-full flex-col p-5 md:p-6">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className={`text-xs font-bold uppercase tracking-[0.22em] ${active ? "text-[#FFD24A]" : "text-white/70"}`}>{entry.audience_type}</span>
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          <span className={`rounded-full border px-3 py-1 text-[11px] font-bold ${active ? "border-[#062b7f]/15 bg-[#eef4ff] text-[#062b7f]" : "border-white/10 bg-white/8 text-white/78"}`}>
            {entry.audience_type === "student" ? "Student review" : "Client review"}
          </span>
          <span className={`rounded-full border px-3 py-1 text-[11px] font-bold ${active ? "border-[#ffd24a]/35 bg-[#fff7db] text-[#8a6400]" : "border-[#ffd24a]/20 bg-[#ffd24a]/12 text-[#ffd24a]"}`}>
            {getFeedbackCategoryLabel(entry.audience_type, entry.category)}
          </span>
        </div>

        <div className={`mb-4 flex items-center gap-2 text-xs font-semibold ${active ? "text-white/72" : "text-white/64"}`}>
          <Icon name="schedule" className="text-sm" />
          <span>{formatRelativeTime(entry.created_at)}</span>
        </div>

        <div className="flex-1">
          {entry.title ? <h4 className={`text-[28px] font-black leading-tight ${active ? "text-white" : "text-white/94"}`}>{entry.title}</h4> : null}
          <p className={`mt-4 text-sm leading-7 ${active ? "text-white/82" : "text-white/76"}`}>“{entry.message}”</p>
        </div>

        <div className={`mt-6 flex items-center justify-between border-t border-white/10 pt-4 text-xs ${active ? "text-white/72" : "text-white/64"}`}>
...
export function FeedbackGallery({
  ...
  if (visibleEntries.length === 0) {
    return (
      <div>
        ...
        <div className="mb-8 flex flex-wrap items-center gap-2.5 overflow-x-auto pb-1">
          ...
          {categories.map((value) => {
            ...
            return (
              ...
              >
                <span>{label}</span>
                <span className={`${countBase} ${selectedCategory === value ? "bg-white/30 text-on-secondary-fixed" : countIdle}`}>
        ...
        <div className="rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,#081638_0%,#050d20_100%)] p-8 text-center text-white shadow-[0_24px_70px_rgba(2,7,27,0.28)]">
          <Icon name="reviews" className="text-4xl text-[#FFD24A]" />
          <h3 className="mt-3 text-xl font-black">No approved feedback yet</h3>
          <p className="mt-2 text-sm text-white/72">Approved stories will appear here after admin review.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="relative mx-auto flex h-[520px] w-full items-center justify-center overflow-hidden rounded-[34px] border border-white/10 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_35%),linear-gradient(180deg,#06122a_0%,#040a18_100%)] px-4 py-6 shadow-[0_30px_100px_rgba(2,7,27,0.28)] sm:h-[560px] sm:px-8">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,210,74,0.12),transparent_42%)]" />

        <button
          type="button"
          onClick={handlePrev}
          className="absolute left-4 z-30 flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/8 text-white backdrop-blur-md transition hover:bg-[#FFD24A] hover:text-[#030B1C] sm:left-6"
        ...
        <button
          type="button"
          onClick={handleNext}
          className="absolute right-4 z-30 flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/8 text-white backdrop-blur-md transition hover:bg-[#FFD24A] hover:text-[#030B1C] sm:right-6"
        ...
        <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2">
          {visibleEntries.map((_, idx) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${idx === activeIndex ? "w-8 bg-[#FFD24A]" : "w-2 bg-white/20 hover:bg-white/45"}`}

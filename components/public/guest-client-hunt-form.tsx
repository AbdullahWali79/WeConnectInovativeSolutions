...
export function GuestClientHuntForm({ form, keywords }: { form: PublicClientHuntForm; keywords: PublicClientHuntKeyword[] }) {
  ...
  return <form onSubmit={submit} className="space-y-4 rounded-2xl border border-outline-variant bg-surface-container-lowest p-3 shadow-xl min-[360px]:p-4 sm:space-y-5 sm:rounded-3xl sm:p-8">
    {keywords.length > 0 ? <section className="rounded-2xl border border-primary/25 bg-gradient-to-r from-primary/10 to-secondary/10 p-4 shadow-sm sm:p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0 w-full sm:flex-1"><p className="text-xs font-black uppercase tracking-[0.16em] text-primary">Your search keyword</p><p className="mt-2 break-words text-xl font-black leading-snug text-on-surface sm:text-2xl">{keywords[keywordIndex]?.keyword}</p><p className="mt-2 text-xs font-bold text-on-surface-variant">Keyword {keywordIndex + 1} of {keywords.length}</p></div>{keywords.length > 1 ? <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto"><button type="button" className="wc-secondary-btn min-w-0 justify-center px-3" disabled={keywordIndex === 0} onClick={() => setKeywordIndex((current) => Math.max(0, current - 1))}>Previous</button><button type="button" className="wc-primary-btn min-w-0 justify-center px-3" disabled={keywordIndex === keywords.length - 1} onClick={() => setKeywordIndex((current) => Math.min(keywords.length - 1, current + 1))}>Next</button></div> : null}</div></section> : <label className="block"><span className="wc-label">Search keyword *</span><input className="wc-input mt-2" value={fields.keywordText} onChange={(e) => update("keywordText", e.target.value)} placeholder="e.g. gym in London" required /></label>}
    ...
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <fieldset className="sm:col-span-2"><legend className="wc-label">Services client needs *</legend><div className="mt-2 grid gap-3 rounded-xl border border-outline-variant p-4 sm:grid-cols-2">{PUBLIC_CLIENT_HUNT_SERVICES.map((service) => <label key={service.value} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-3 font-semibold transition-colors ${fields.servicesRequired.includes(service.value) ? "border-primary bg-primary/10" : "border-slate-200 bg-white hover:border-primary/50 hover:bg-slate-50"}`}><input type="checkbox" className="h-5 w-5 shrink-0 rounded border-2 border-slate-500 bg-white text-primary accent-blue-700 focus:ring-2 focus:ring-primary focus:ring-offset-2 checked:border-primary checked:bg-primary" style={{ accentColor: "var(--wc-primary)" }} checked={fields.servicesRequired.includes(service.value)} onChange={(event) => setFields((current) => ({ ...current, servicesRequired: event.target.checked ? [...current.servicesRequired, service.value] : current.servicesRequired.filter((value) => value !== service.value) }))} /><span>{service.label}</span></label>)}</div></fieldset>
    </div>

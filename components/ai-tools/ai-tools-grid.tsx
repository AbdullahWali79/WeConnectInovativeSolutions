...
export function AIToolsGrid({ tools }: { tools: AITool[] }) {
  ...
  return <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">{tools.map((tool) => <article key={tool.id} className="wc-card group overflow-hidden">
    ...
    <div className="mt-5 flex flex-wrap gap-2"><a href={tool.url} target="_blank" rel="noopener noreferrer" className="wc-primary-btn inline-flex text-sm"><Icon name="open_in_new" /> Explore tool</a>{tool.youtube_url && <a href={tool.youtube_url} target="_blank" rel="noopener noreferrer" className="wc-secondary-btn inline-flex text-sm"><Icon name="play_circle" /> Learn on YouTube</a>}</div></div>
  </article>)}</div>;
}

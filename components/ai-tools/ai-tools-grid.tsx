...
export function AIToolsGrid({ tools }: { tools: AITool[] }) {
  ...
  return <div className="space-y-6">
    ...
    {filteredTools.length ? <div className="grid items-start gap-6 sm:grid-cols-2 xl:grid-cols-3">{filteredTools.map((tool) => <ToolCard key={tool.id} tool={tool} onSelect={() => setSelected(tool)} />)}</div> : <EmptyState hasTools={tools.length > 0} />}
    {selected && <ToolDetailsModal tool={selected} onClose={() => setSelected(null)} />}
  </div>;
}
...
function ToolCard({ tool, onSelect }: { tool: AITool; onSelect: () => void }) {
  return <article className="wc-card group cursor-pointer overflow-hidden transition duration-300 hover:-translate-y-1 hover:shadow-xl" onClick={onSelect}>
    ...
    <div className="p-5">
      <div className="flex items-start justify-between gap-3"><h2 className="min-w-0 text-xl font-black leading-tight text-on-surface">{tool.name}</h2><span className="shrink-0 rounded-full bg-secondary-container px-2.5 py-1 text-[10px] font-black uppercase text-on-secondary-container">Verified</span></div>
...
function ToolDetailsModal({ tool, onClose }: { tool: AITool; onClose: () => void }) {
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 p-4 backdrop-blur-md sm:p-6" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="ai-tool-title">
    <div className="relative flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-outline-variant bg-surface shadow-2xl" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-center justify-between border-b border-outline-variant bg-surface/95 p-5 backdrop-blur-xl sm:p-6">
        <div className="min-w-0"><p className="mb-1 text-[10px] font-black uppercase tracking-widest text-secondary">Verified AI Tool</p><h2 id="ai-tool-title" className="truncate text-2xl font-black text-on-surface">{tool.name}</h2></div>
